...
describe('splitDocuments', () => {
  it('splits a single document into multiple chunks and preserves metadata', async () => {
    ...
    const doc = new Document({ pageContent: longText, metadata: { articleId: 10, articleTitle: 'Chunk Test', categoryPath: '/cat' } });
  ...
  it('overlaps chunks correctly with chunkOverlap=150', async () => {
    ...
    const doc = new Document({ pageContent: longText, metadata: { articleId: 11, articleTitle: 'Overlap', categoryPath: '/overlap' } });
    const chunks = await splitDocuments([doc]);
    if (chunks.length >= 2) {
