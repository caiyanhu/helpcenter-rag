...
export async function saveOutlineTree(outlineTree: OutlineNode, categoryId: number): Promise<void> {
  ...
  await fs.writeFile(
    ...
    JSON.stringify(outlineTree, null, 2)
  );
}

export function buildCategoryPath(node: OutlineNode, targetId: number, currentPath: string[] = []): string[] | null {
...
export async function loadCachedArticles(categoryId: number): Promise<CachedArticle[]> {
  ...
  async function walkDir(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      } else if (entry.isFile() && entry.name.endsWith('.json') && entry.name !== 'category.json' && entry.name !== 'outline.json') {
        try {
